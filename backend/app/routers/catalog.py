...
def _bypass_price_sensitive_cache(
    *,
    ...
    if price_min is not None or price_max is not None:
        return True
    if os.getenv("CATALOG_PRICE_SORT_CACHE_BYPASS", "0") != "1":
        return False
...
def list_cars(
    ...
    interior_color: Optional[str] = Query(default=None),
    interior_material: Optional[str] = Query(default=None),
    vat_reclaimable: Optional[str] = Query(default=None),
...
def cars_count(
    ...
    source: Optional[str | List[str]] = Query(default=None),
    num_seats: Optional[str] = Query(default=None),
    doors_count: Optional[str] = Query(default=None),
    emission_class: Optional[str] = Query(default=None),
    efficiency_class: Optional[str] = Query(default=None),
...
def advanced_count(request: Request, db: Session = Depends(get_db)):
    ...
    raw_params = {
        ...
        "vat_reclaimable": qp.get("vat_reclaimable"),
