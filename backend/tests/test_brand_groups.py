...
def test_group_brands_keeps_priority_order_then_alpha():
    ...
    assert groups["other"] == ["Aston Martin", "BMW", "Dacia", "Renault"]


def test_group_brands_normalizes_aliases_to_priority_brand():
    groups = group_brands(["mercedes", "ROLLS ROY'S", "Land-Rover", "vw"])
    assert groups["top"] == [
        "Mercedes-Benz",
        "Volkswagen",
        "Land Rover",
