...
def test_catalog_inline_price_refresh_defaults_to_opt_in(monkeypatch):
    ...
    assert service._should_catalog_inline_price_refresh(page=3, page_size=24) is False


def test_catalog_inline_price_refresh_can_be_enabled_for_visible_pages(monkeypatch):
    monkeypatch.delenv("CATALOG_INLINE_PRICE_REFRESH", raising=False)
    monkeypatch.setenv("CATALOG_INLINE_PRICE_REFRESH_DEFAULT", "1")
    monkeypatch.delenv("CATALOG_INLINE_PRICE_REFRESH_MAX_PAGE", raising=False)
    monkeypatch.delenv("CATALOG_INLINE_PRICE_REFRESH_MAX_PAGE_SIZE", raising=False)
    service = CarsService(db=None)  # type: ignore[arg-type]
